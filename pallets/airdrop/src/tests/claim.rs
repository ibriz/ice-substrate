...
fn dispatch_claim_flow() {
	...
	test_ext.execute_with(||{
		...
		let icon_signature = bytes::from_hex(VALID_ICON_SIGNATURE).unwrap();
			...
			let ice_address = <mock::Test as frame_system::Config>::AccountId::decode(&mut &ice_bytes[..]).unwrap_or_default();
		...
		assert_ok!(AirdropModule::dispatch_claim(
			Origin::signed(AirdropModule::get_offchain_account().unwrap()),
			icon_wallet,
			ice_address.clone(),
			message.to_vec(),
